export const apps = [
  {
    name: "Signal",
    category: "centralized",
    tagline: "Gold standard E2EE, centralized servers",
    icon: "🔵",
    protocol: "Signal Protocol (Double Ratchet)",
    architecture: "Centralized",
    encryption: "E2E (Signal Protocol + PQXDH for initial key exchange since Sept 2023)",
    openSource: "Client: Yes, Server: Yes",
    openSourceStatus: true,
    identifier: "Phone number required",
    metadataProtection: "Minimal — server sees who talks to whom, when, IP addresses",
    pfs: true,
    postCompromise: true,
    quantumResistant: null,
    anonymousSignup: false,
    noPhoneRequired: false,
    selfHostable: false,
    federated: false,
    p2p: false,
    offlineCapable: false,
    groupSize: "Up to 1,000 members",
    voiceVideo: true,
    multiDevice: true,
    platforms: "Android, iOS, Windows, macOS, Linux",
    cellTooltips: {
      quantumResistant: "PQXDH post-quantum key exchange is used only for the initial session setup (deployed Sept 2023). The ongoing Double Ratchet messages are not yet quantum-resistant.",
    },
    funding: "Non-profit (Signal Foundation, Brian Acton donation)",
    jurisdiction: "USA (Five Eyes)",
    audited: true,
    userBase: "~40M+ monthly users",
    price: "Free",
    maxDevices: "5 linked (phone + up to 4 companion devices)",
    rateLimiting: "Server-side, closed-source. Behavior-based detection + CAPTCHA challenges. Exact rates unpublished; reports suggest ~10 msgs before limits trigger for new contacts. Spam detection code kept private intentionally.",
    offlineDelivery: "Central servers queue messages. Recipient pulls queued messages on reconnect. Simple, reliable — the server is the store-and-forward node. This is exactly the centralization point.",
    pros: [
      "Best-in-class encryption protocol, widely adopted",
      "PQXDH post-quantum key exchange for initial session establishment (deployed Sept 2023)",
      "Excellent UX, familiar WhatsApp-like interface",
      "Robust voice/video calling",
      "Independent non-profit with no ad/data business model",
      "Protocol used by WhatsApp, Google Messages, etc.",
    ],
    cons: [
      "Requires phone number — no anonymous sign-up",
      "Centralized servers = single point of failure/control",
      "US jurisdiction (Five Eyes alliance)",
      "Doesn't allow third-party clients or federation",
      "Server sees metadata: who talks to whom, when, IP addresses",
      "Account recovery tied to phone number",
    ],
    centralizationPoints: [
      "Single server infrastructure operated by Signal Foundation",
      "Phone number as mandatory identifier",
      "Google/Apple push notification dependency",
      "No federation — walled garden",
    ],
  },
  {
    name: "Matrix / Element",
    category: "federated",
    tagline: "Federated protocol, run your own server",
    icon: "🟢",
    protocol: "Matrix (Olm/Megolm, Double Ratchet variant)",
    architecture: "Federated",
    encryption: "E2E (Megolm) — default for DMs since 2023, opt-in for rooms",
    openSource: "Client: Yes, Server: Yes, Protocol: Yes",
    openSourceStatus: true,
    identifier: "Matrix ID (@user:server.org)",
    metadataProtection: "Weak — homeservers see metadata, rooms replicate across servers",
    pfs: true,
    postCompromise: true,
    quantumResistant: false,
    anonymousSignup: true,
    noPhoneRequired: true,
    selfHostable: true,
    federated: true,
    p2p: false,
    offlineCapable: false,
    groupSize: "No protocol limit; homeserver-dependent (thousands in practice)",
    voiceVideo: true,
    multiDevice: true,
    platforms: "Android, iOS, Windows, macOS, Linux, Web",
    funding: "Element (VC-backed company) + Matrix.org Foundation (non-profit)",
    jurisdiction: "UK (Five Eyes)",
    audited: true,
    userBase: "~100M+ accounts across federation",
    price: "Free (self-host) / Element One from €5/mo",
    maxDevices: "Unlimited (per-device sessions)",
    rateLimiting: "Server-configurable. Synapse defaults: 0.2 msg/sec sustained, burst of 10. Admins can tune per-user, per-room, per-IP. Covers messages, registrations, joins, invites. No protocol-level enforcement — each homeserver sets its own rules.",
    offlineDelivery: "Recipient's homeserver stores messages in the room's event DAG. On reconnect, client syncs via /sync. Room replication across all participating homeservers means the message exists on multiple servers — even if sender's homeserver dies. Very robust, but messages persist on multiple servers (bad for metadata).",
    pros: [
      "True federation — choose or run your own homeserver",
      "Rich ecosystem: bridges to Slack, Discord, IRC, Telegram, etc.",
      "Excellent for organizations & communities (Spaces, threads)",
      "Fully open protocol — anyone can build clients/servers",
      "Used by governments (France, Germany) and NATO",
      "Room replication provides redundancy",
    ],
    cons: [
      "Metadata leaks heavily — homeservers see everything",
      "E2EE historically flaky, key verification UX is poor",
      "Synapse server is resource-hungry and complex to self-host",
      "Federation creates a large attack surface",
      "Most users default to matrix.org = de facto centralization",
      "Slow protocol evolution, complex spec",
    ],
    centralizationPoints: [
      "matrix.org homeserver hosts majority of users",
      "Element company dominates client/server development",
      "Matrix.org Foundation controls the spec",
      "Key management/cross-signing UX pushes users to defaults",
    ],
  },
  {
    name: "Session",
    category: "decentralized",
    tagline: "Onion-routed, no phone number, blockchain nodes",
    icon: "🟣",
    protocol: "Session Protocol (Libsodium-based)",
    architecture: "Decentralized (blockchain node network)",
    encryption: "E2E (Session Protocol)",
    openSource: "Client: Yes, Node: Yes",
    openSourceStatus: true,
    identifier: "Random 66-char Session ID",
    metadataProtection: "Strong — onion routing hides IP, no phone/email required",
    pfs: false,
    postCompromise: false,
    quantumResistant: false,
    anonymousSignup: true,
    noPhoneRequired: true,
    selfHostable: false,
    federated: false,
    p2p: false,
    offlineCapable: false,
    groupSize: "100 (closed groups), unlimited (Communities/Open Groups)",
    voiceVideo: true,
    multiDevice: true,
    platforms: "Android, iOS, Windows, macOS, Linux",
    cellTooltips: {
      pfs: "Session dropped the Signal Protocol in favor of its own Session Protocol (NaCl-based), which uses static keys without ephemeral key ratcheting.",
      postCompromise: "Without PFS there's no automatic session healing. A captured device key exposes all messages protected by it until keys are manually regenerated.",
    },
    funding: "Session Technology Foundation + OXEN/SESSION token",
    jurisdiction: "Switzerland (moved from Australia in 2024)",
    audited: true,
    userBase: "~1M+ users",
    price: "Free",
    maxDevices: "Multi-device (up to a few linked devices via sync)",
    rateLimiting: "Service node level. Swarm nodes may reject excessive requests but no formal protocol-level rate limiting. Closed groups limited to ~100 members which implicitly caps message fan-out. No published rate numbers.",
    offlineDelivery: "Swarm architecture. Each Session ID is assigned to a swarm of 5–9 Service Nodes. Messages deposited via onion routing into recipient's swarm, stored redundantly for up to 14 days. Recipient polls swarm on reconnect, messages deleted after retrieval. Nodes act as temporary store-and-forward relays.",
    pros: [
      "No phone number or email required — truly anonymous sign-up",
      "Onion routing hides IP addresses from all participants",
      "Decentralized node network (~2000+ nodes) with no central server",
      "Moved to Switzerland to escape Five Eyes jurisdiction",
      "Simple UX despite decentralized architecture",
      "Account restoration with seed phrase",
    ],
    cons: [
      "No Perfect Forward Secrecy (PFS) — major cryptographic weakness",
      "No post-compromise security (break-in recovery)",
      "Dropped Signal Protocol for weaker custom protocol",
      "Relies on blockchain staking (OXEN/SESSION token) for node network",
      "File server is still centralized (though accessed via onion requests)",
      "Small user base, group size limited to ~100",
      "Voice/video call quality reported as poor",
    ],
    centralizationPoints: [
      "Centralized file/attachment server (accessed via onion routing)",
      "Push notification service is centralized",
      "SESSION token economics influence node network",
      "Session Technology Foundation controls development",
    ],
  },
  {
    name: "SimpleX Chat",
    category: "decentralized",
    tagline: "No user identifiers at all — not even random ones",
    icon: "⚫",
    protocol: "SimpleX Messaging Protocol (SMP) + Double Ratchet",
    architecture: "Decentralized (relay servers, no global identity)",
    encryption: "E2E (Double Ratchet + post-quantum hybrid)",
    openSource: "Client: Yes, Server: Yes, Protocol: Yes",
    openSourceStatus: true,
    identifier: "None — uses per-connection queue addresses",
    metadataProtection: "Very strong — no user IDs, no contact graph visible to servers",
    pfs: true,
    postCompromise: true,
    quantumResistant: true,
    anonymousSignup: true,
    noPhoneRequired: true,
    selfHostable: true,
    federated: false,
    p2p: false,
    offlineCapable: false,
    groupSize: "No hard limit; performance degrades at scale (relay-based, O(n²) connections)",
    voiceVideo: true,
    multiDevice: true,
    platforms: "Android, iOS, Windows, macOS, Linux, Terminal CLI",
    cellTooltips: {
      quantumResistant: "Hybrid X25519 + Kyber-1024 key agreement: simultaneously resistant to quantum and classical attackers without sacrificing performance.",
    },
    funding: "Donations + VC (Jack Dorsey, Asymmetric) — no token",
    jurisdiction: "UK (company), open protocol",
    audited: true,
    userBase: "~Hundreds of thousands",
    price: "Free (open source)",
    maxDevices: "Multiple (up to ~8, each gets independent keys)",
    rateLimiting: "SMP relay level. Relays can enforce per-connection queue rate limits. No published default rates. Self-hosted relays = your own rules. No protocol-level spam protection beyond queue-based isolation (each contact uses separate queue).",
    offlineDelivery: "SMP relay servers hold messages in per-connection queues. Relay chosen by recipient (not sender) stores message until retrieval, then deletes. Relay knows queue ID only, not sender identity. Recipient can self-host relay or use defaults.",
    pros: [
      "No user identifiers whatsoever — revolutionary privacy model",
      "Double Ratchet with post-quantum hybrid encryption",
      "Servers cannot see who talks to whom (no contact graph)",
      "Self-hostable relay servers, users choose their servers",
      "Content padding conceals message size",
      "Tor-compatible for additional anonymity",
      "Active development with regular security audits (Trail of Bits)",
      "No blockchain, no token — funded by donations and VC",
    ],
    cons: [
      "Adding contacts requires out-of-band link/QR exchange",
      "Most users use default SimpleX servers = practical centralization",
      "UK jurisdiction raises concerns (Online Safety Act debates)",
      "Smaller community, fewer network effects",
      "Video call quality can be buggy",
      "Complex architecture harder to audit holistically",
      "Group management can have consistency issues in decentralized state",
    ],
    centralizationPoints: [
      "Default relay servers hosted by SimpleX Chat Ltd",
      "SimpleX Chat Ltd (UK) controls development direction",
      "XFTP file transfer servers",
      "VC funding (Jack Dorsey) may influence future direction",
    ],
  },
  {
    name: "Briar",
    category: "p2p",
    tagline: "True P2P: Tor, Bluetooth, Wi-Fi — no servers at all",
    icon: "🟤",
    protocol: "Bramble Transport Protocol + Bramble Sync Protocol",
    architecture: "Pure P2P (Tor, Bluetooth, Wi-Fi direct)",
    encryption: "E2E (custom protocol over Tor)",
    openSource: "Client: Yes, Protocol: Yes",
    openSourceStatus: true,
    identifier: "Briar identity key (exchanged in person or via link)",
    metadataProtection: "Excellent — no servers, messages stay on device",
    pfs: true,
    postCompromise: true,
    quantumResistant: false,
    anonymousSignup: true,
    noPhoneRequired: true,
    selfHostable: false,
    federated: false,
    p2p: true,
    offlineCapable: true,
    groupSize: "Up to 50 members (private groups/forums)",
    voiceVideo: false,
    multiDevice: false,
    platforms: "Android only (desktop in beta)",
    cellTooltips: {
      offlineCapable: "Communicates over Bluetooth and Wi-Fi Direct when no internet is available. Designed for activists in connectivity-denied or censored environments.",
      multiDevice: "By design: all data stays on-device with no cloud sync. Sharing state across devices would require a server component that contradicts Briar's threat model.",
    },
    funding: "Grants (Open Technology Fund, etc.) + donations",
    jurisdiction: "Open-source project (international contributors)",
    audited: true,
    userBase: "~50K-100K",
    price: "Free (open source)",
    maxDevices: "1 (no multi-device support)",
    rateLimiting: "None. Direct P2P connections only — no intermediary to enforce limits. Bluetooth/Wi-Fi range and Tor circuit limits provide implicit throughput caps. No spam protection mechanism.",
    offlineDelivery: "Original design requires both online simultaneously. Added mailbox feature: a trusted contact's always-on device (e.g., Raspberry Pi) receives messages on your behalf. Honest acknowledgment that store-and-forward requires something online — but major UX limitation.",
    pros: [
      "Zero servers — true peer-to-peer architecture",
      "Works without internet via Bluetooth/Wi-Fi mesh",
      "Messages stored only on user devices, never in cloud",
      "Routes over Tor when internet is available",
      "Perfect for activists, journalists in hostile environments",
      "No metadata for anyone to collect",
      "Independent security audit completed",
    ],
    cons: [
      "Android only (Linux desktop in beta, no iOS)",
      "No voice or video calls",
      "Both users must be online simultaneously for delivery (or in BT range)",
      "Text-only messages (no files beyond images in forums)",
      "Battery-intensive due to Tor and mesh networking",
      "Very small user base and limited functionality",
      "Contact exchange requires in-person meeting or link sharing",
      "No multi-device support",
    ],
    centralizationPoints: [
      "Tor network dependency (Tor has directory authorities)",
      "Small development team with limited resources",
      "Google Play Store distribution (F-Droid available as alternative)",
    ],
  },
  {
    name: "Status",
    category: "decentralized",
    tagline: "Waku protocol + crypto wallet + Web3 browser",
    icon: "🔷",
    protocol: "Waku v2 (libp2p pub/sub) + Double Ratchet",
    architecture: "Decentralized (P2P via Waku network)",
    encryption: "E2E (Double Ratchet over Waku)",
    openSource: "Client: Yes, Protocol: Yes (Waku)",
    openSourceStatus: true,
    identifier: "Ethereum keypair (chat key)",
    metadataProtection: "Moderate-to-good — P2P pub/sub, no central server stores messages",
    pfs: true,
    postCompromise: true,
    quantumResistant: false,
    anonymousSignup: true,
    noPhoneRequired: true,
    selfHostable: "partial",
    federated: false,
    p2p: true,
    offlineCapable: false,
    groupSize: "Closed groups: up to 20; Communities: thousands (Waku pub/sub)",
    voiceVideo: false,
    multiDevice: true,
    platforms: "Android, iOS, Windows, macOS, Linux",
    cellTooltips: {
      selfHostable: "Anyone can run Waku relay nodes, contributing to message routing. But the Status app itself connects to IFT-operated fleet nodes by default — there's no one-click self-hosted Status stack.",
    },
    funding: "IFT (Institute of Free Technology) / Status Network + SNT token",
    jurisdiction: "Switzerland (Logos/IFT)",
    audited: true,
    userBase: "~100K-500K",
    price: "Free",
    maxDevices: "Multiple (Waku key sync; no hard cap published)",
    rateLimiting: "No RLN on Status's Waku fleet — RLN is a Waku protocol feature but is not enabled in Status's production deployment. Relay nodes apply basic connection-level controls only. No protocol-level rate enforcement; spam protection relies on application-layer heuristics.",
    offlineDelivery: "Waku Store protocol. Messages propagate through relay network (libp2p GossipSub), Store nodes persist messages. Recipient queries Store nodes on reconnect. SDS (Scalable Data Sync) adds reliability layer — detects gaps, backfills from Store nodes. Store node incentivization still in development (ZK ticket-based payments PoC).",
    pros: [
      "Built on Waku — communication layer of the Logos tech stack",
      "Part of Ethereum's original 'Holy Trinity' vision (compute, storage, communication)",
      "Integrated crypto wallet and Web3 browser",
      "Communities feature for large decentralized groups",
      "No phone number required, Ethereum key-based identity",
      "Active protocol research (mixnet, SDS reliability, incentivization)",
      "Recognized by Vitalik Buterin as continuation of Whisper",
    ],
    cons: [
      "Waku protocol still maturing — message reliability issues being worked on",
      "App can be resource-heavy and battery-intensive",
      "Smaller user base compared to Signal/Matrix",
      "Complexity of crypto/Web3 features may alienate non-crypto users",
      "SNT token economics add financial complexity to ecosystem",
      "No voice/video calls yet",
      "Incentivization model for node operators still in development",
    ],
    centralizationPoints: [
      "IFT/Status Network funds and directs core development",
      "Default Waku nodes (though anyone can run nodes)",
      "Infura and similar services for Ethereum RPC",
      "App store distribution (Google Play, Apple App Store)",
    ],
  },
  {
    name: "Logos Chat (Chat SDK)",
    category: "prerelease",
    tagline: "Noise + Double Ratchet + de-MLS over Waku — SDK for developers",
    icon: "🔶",
    protocol: "Noise (session) + Double Ratchet (1:1) + de-MLS (groups) over Waku v2",
    architecture: "Decentralized (P2P via Waku/Logos Messaging network)",
    encryption: "E2E: Noise handshakes → Double Ratchet (1:1), de-MLS (groups) — in development",
    openSource: "SDK: Yes (Rust, with Nim bindings via libchat), Protocol: Yes, Nodes: Yes (nwaku/js-waku)",
    openSourceStatus: true,
    identifier: "Crypto keypair + Ratcheting Private Identifiers (draft)",
    metadataProtection: "Strong (design target) — pub/sub routing, no central server, mixnet in testing",
    pfs: true,
    postCompromise: true,
    quantumResistant: "planned",
    anonymousSignup: true,
    noPhoneRequired: true,
    selfHostable: "partial",
    federated: false,
    p2p: true,
    offlineCapable: false,
    groupSize: "No fixed limit (design goal); de-MLS provides O(log N) key operations — pre-release, unverified",
    voiceVideo: false,
    multiDevice: true,
    platforms: "Rust (native/mobile via libchat), JS/TS (browser via js-waku), Android, iOS (mobile planned for Mainnet)",
    cellTooltips: {
      selfHostable: "The SDK integrates with any nwaku/js-waku node you run. But it's a developer SDK, not a self-contained server installation — developer setup required.",
      quantumResistant: "Kyber KEM post-quantum upgrade is on the roadmap but not yet implemented in the current pre-release SDK.",
      audited: "An independent security audit is required and planned before mainnet. Currently pre-release, developer-only testing phase.",
    },
    funding: "IFT / Logos Collective (no token dependency for SDK)",
    jurisdiction: "Switzerland (IFT)",
    audited: false,
    userBase: "Pre-release — developer/dogfooding only",
    price: "Free (open-source SDK)",
    maxDevices: "Multiple (MLS-based; design supports many devices)",
    rateLimiting: "RLN (same as Waku) — ZK proof-based, privacy-preserving. SDK includes rate limit manager backed by SQLite. Configurable per-membership limits. Integrated into Reliable Channel API layer. Designed to be transparent to app developers.",
    offlineDelivery: "Same as Status/Waku: Waku Store protocol + SDS reliability layer. The Chat SDK integrates SDS at the Reliable Channel API level, handling gap detection and backfill transparently. Store node economics still being designed.",
    maturity: "MVP in development",
    pros: [
      "External security audit required before Mainnet — internal review + independent audit planned",
      "Cleanest layered architecture: Waku transport → Reliable Channel → Chat SDK → App",
      "de-MLS for groups: MLS scalability without centralized Delivery Service (Vac research)",
      "Noise + Double Ratchet = proven crypto foundations, not reinventing the wheel",
      "RLN for privacy-preserving DoS protection — unique in the messaging space",
      "Designed for third-party developers from day one, not just one app's internal protocol",
      "Mixnet integration (libp2p mix) already being tested for traffic analysis resistance",
      "Ratcheting Private Identifiers — rotating identity markers to prevent long-term correlation",
      "SDS (Scalable Data Sync) for end-to-end message reliability with gap detection",
      "WebRTC signalling over Waku being explored for low-latency browser P2P",
      "Part of broader Logos stack (Nomos consensus + Codex storage + Waku messaging)",
    ],
    cons: [
      "Not yet at mainnet — testnet phases (v0.1–v0.3) preceding GA",
      "de-MLS Steward role is semi-centralized for group initialization",
      "Nim ecosystem is small — fewer auditors, fewer contributors, harder hiring",
      "Extreme complexity: Waku + SDS + RLN + Noise + DR + de-MLS + segmentation + sharding",
      "Key backup/recovery design still being finalized",
      "Development priorities may get pulled toward Status's specific needs",
      "No voice/video calls",
      "Store node incentivization still PoC (ZK ticket-based payments)",
    ],
    centralizationPoints: [
      "IFT/Logos controls development direction and funding",
      "Waku fleet nodes used for bootstrapping/peer discovery (not message routing)",
      "Store nodes are centralized in practice — no live incentivization, IFT operates default nodes",
      "de-MLS Steward role for group membership management",
      "RLN membership on Linea Sepolia testnet — chain dependency",
    ],
  },
  {
    name: "XMPP (Conversations, etc.)",
    category: "federated",
    tagline: "The OG federated protocol — 25+ years old",
    icon: "🟠",
    protocol: "XMPP + OMEMO (Double Ratchet variant)",
    architecture: "Federated",
    encryption: "E2E via OMEMO extension (opt-in)",
    openSource: "Clients: Many, Servers: Many, Protocol: Yes (IETF standard)",
    openSourceStatus: true,
    identifier: "JID (user@server.tld)",
    metadataProtection: "Weak — server admin sees metadata, federation leaks routing info",
    pfs: true,
    postCompromise: true,
    quantumResistant: false,
    anonymousSignup: true,
    noPhoneRequired: true,
    selfHostable: true,
    federated: true,
    p2p: false,
    offlineCapable: false,
    groupSize: "Server-configurable; Prosody default: 200, ejabberd: thousands",
    voiceVideo: true,
    multiDevice: true,
    platforms: "Android, iOS, Windows, macOS, Linux, Web (many clients)",
    cellTooltips: {
      audited: "There's no single XMPP codebase to audit — XMPP is an IETF standard. Each client (Conversations, Gajim) and server (Prosody, ejabberd) has its own varying audit status.",
    },
    funding: "Volunteer-driven + XMPP Standards Foundation (non-profit)",
    jurisdiction: "Decentralized (no single entity)",
    audited: false,
    userBase: "Millions (fragmented across servers)",
    price: "Free (self-host or use public servers)",
    maxDevices: "Unlimited (multiple OMEMO device keys per JID)",
    rateLimiting: "Server-configurable. Varies by implementation — ejabberd and Prosody both support rate limiting modules. No protocol-level standard. Typically configured per-connection or per-JID by server admin. No default rates in the XMPP spec itself.",
    offlineDelivery: "Server stores offline messages (XEP-0160), delivers on reconnect. MAM (Message Archive Management, XEP-0313) provides persistent server-side history. Straightforward server-side storage — your server admin holds your messages.",
    pros: [
      "IETF standard — longest-running federated messaging protocol",
      "Massive client/server ecosystem, many choices",
      "True federation — full interoperability between servers",
      "OMEMO provides solid E2E encryption",
      "Easy to self-host (ejabberd, Prosody are lightweight)",
      "No single company controls the protocol",
      "Extremely extensible via XEPs (XMPP Extension Protocols)",
    ],
    cons: [
      "Fragmented ecosystem — inconsistent client quality and feature support",
      "OMEMO not enabled by default, multi-device key management is painful",
      "UX generally poor compared to modern messengers",
      "Group chat support (MUC) is dated and clunky",
      "Metadata visible to server admins",
      "Voice/video support inconsistent across clients",
      "Declining user base and developer momentum",
    ],
    centralizationPoints: [
      "Large public servers (like jabber.org) host many users",
      "XMPP Standards Foundation controls XEP process",
      "Google dropped XMPP federation (Talk/Hangouts), fragmenting the network",
      "Server admin has full metadata access",
    ],
  },
  {
    name: "Tox",
    category: "p2p",
    tagline: "Serverless P2P with DHT — zero infrastructure",
    icon: "🟡",
    protocol: "Tox protocol (NaCl encryption + DHT)",
    architecture: "Pure P2P (DHT-based)",
    encryption: "E2E (NaCl/libsodium)",
    openSource: "Client: Yes (multiple), Protocol: Yes",
    openSourceStatus: true,
    identifier: "Tox ID (76-char hex string)",
    metadataProtection: "Moderate — DHT exposes IP addresses to peers",
    pfs: false,
    postCompromise: false,
    quantumResistant: false,
    anonymousSignup: true,
    noPhoneRequired: true,
    selfHostable: false,
    federated: false,
    p2p: true,
    offlineCapable: false,
    groupSize: "Up to 512 members",
    voiceVideo: true,
    multiDevice: false,
    platforms: "Android, iOS, Windows, macOS, Linux",
    cellTooltips: {
      pfs: "Tox uses static Curve25519 keys per session with no key ratcheting. The same session key protects all messages until manually regenerated.",
    },
    funding: "Community-driven, donations",
    jurisdiction: "Open-source project (no legal entity)",
    audited: false,
    userBase: "~10K-50K active",
    price: "Free (open source)",
    maxDevices: "1 (no multi-device support)",
    rateLimiting: "None formal. DHT protocol has implicit bandwidth limits. No spam protection mechanism at protocol level. Accepting a friend request is the only gate. Once connected, no rate enforcement.",
    offlineDelivery: "No solution. Both peers must be online simultaneously. No store-and-forward in protocol. Client retries when it detects peer's DHT status changes to online. Worst case for offline delivery among all compared apps.",
    pros: [
      "Completely serverless — no infrastructure to compromise",
      "No sign-up, no phone, no email — instant start",
      "Built-in voice and video calls (P2P)",
      "Multiple client implementations (qTox, Toxygen, etc.)",
      "Strong encryption (NaCl/libsodium)",
    ],
    cons: [
      "No Perfect Forward Secrecy",
      "IP addresses potentially exposed via DHT",
      "Both peers must be online for message delivery",
      "Susceptible to Sybil attacks via DHT",
      "Development has slowed significantly",
      "No multi-device support",
      "UX is rough and inconsistent across clients",
      "Tiny user base",
    ],
    centralizationPoints: [
      "DHT bootstrap nodes are semi-centralized entry points",
      "Development largely stalled — bus factor risk",
      "No formal organization or sustained funding",
    ],
  },
  {
    name: "DarkFi (DarkIRC)",
    category: "p2p",
    tagline: "Fully anonymous P2P IRC — unlinkable messages, ZK-native",
    icon: "⬛",
    protocol: "Custom DAG-based P2P protocol + Signal-like encryption",
    architecture: "Pure P2P (DAG-based event graph, no servers)",
    encryption: "E2E: Signal-like algorithms for DMs, shared-secret symmetric for channels",
    openSource: "Everything: Yes (GNU AGPL, Rust, Codeberg)",
    openSourceStatus: true,
    identifier: "None — no persistent identity, nick changeable at will",
    metadataProtection: "Maximum — messages are unlinkable, no persistent identity, Tor/Nym opt-in",
    pfs: false,
    postCompromise: false,
    quantumResistant: false,
    anonymousSignup: true,
    noPhoneRequired: true,
    selfHostable: true,
    federated: false,
    p2p: true,
    offlineCapable: true,
    groupSize: "No limit for public channels (IRC-style); private groups in development",
    voiceVideo: false,
    multiDevice: false,
    platforms: "Android, Linux, Windows, macOS (custom UI, single codebase)",
    cellTooltips: {
      pfs: "Public channels use a shared symmetric secret, not per-message ephemeral keys. DM encryption follows Signal-like algorithms but PFS is not yet fully implemented.",
      offlineCapable: "DAG-based event graph persists messages for ~24 hours. Nodes sync the DAG on reconnect and retrieve missed messages automatically.",
    },
    funding: "Community-driven, crypto-anarchist collective",
    jurisdiction: "No legal entity — anonymous developer collective",
    audited: false,
    userBase: "Small — alpha stage, dev community + early adopters",
    price: "Free (open source, GNU AGPL)",
    maxDevices: "1 (no multi-device support)",
    rateLimiting: "None formal. DAG-based architecture provides some implicit limits (event graph propagation). No protocol-level spam protection. P2P network topology limits throughput naturally. Planned but not implemented.",
    offlineDelivery: "DAG-based event graph provides ~1 day message permanence. When a node joins the network, it syncs the DAG and retrieves missed messages. No infinite storage — messages eventually expire. Better than pure P2P (Tox) but not indefinite.",
    maturity: "Alpha (Jan 2025 release)",
    pros: [
      "Strongest anonymity claim in the space — messages are fully unlinkable between each other",
      "No persistent identity at all — no accounts, no keys tied to you, change nick freely",
      "DAG-based architecture provides eventual consistency and message permanence (~1 day)",
      "Tor and Nym integration for network-level anonymity",
      "Part of broader DarkFi L1 blockchain with ZK smart contracts, DAO, wallet",
      "Custom cross-platform UI — single codebase for all platforms including mobile",
      "GNU AGPL — strongest copyleft, fully open source on Codeberg (not GitHub)",
      "Kyber post-quantum KEM implemented as standalone crate (future DM encryption)",
      "Pluggable transports architecture — clearnet, Tor, Nym",
      "Super-app vision: chat is first add-on, with DAO/wallet/filesharing planned",
    ],
    cons: [
      "Alpha software — expect bugs, sync can be slow",
      "Private DMs and secret channels still being developed (next milestone)",
      "No PFS or post-compromise security in current channel encryption (shared secret)",
      "Very small network — handful of active users currently",
      "IRC-style UX — channels only, no modern chat features (reactions, threads, etc.)",
      "No voice/video calls",
      "No multi-device support",
      "Requires running a local daemon (darkirc) — not plug-and-play for non-technical users",
      "Crypto-anarchist framing may limit mainstream adoption",
      "No independent security audit",
    ],
    centralizationPoints: [
      "Seed nodes for P2P bootstrapping (standard for any P2P network)",
      "Small core developer team led by Amir Taaki",
      "No formal organization — bus factor risk similar to Tox",
      "Testnet blockchain (DarkFi L1) not yet at mainnet",
    ],
  },
  {
    name: "XMTP",
    category: "web3",
    tagline: "Web3-native messaging with quantum-resistant E2EE",
    icon: "🔴",
    protocol: "XMTP (MLS-based, quantum-resistant hybrid encryption)",
    architecture: "Decentralized (distributed node network)",
    encryption: "E2E (MLS with quantum-resistant hybrid)",
    openSource: "Client SDKs: Yes, Nodes: Yes, Contracts: Yes",
    openSourceStatus: true,
    identifier: "Any DID or wallet address",
    metadataProtection: "Moderate — nodes route messages, payments flow to operators",
    pfs: true,
    postCompromise: true,
    quantumResistant: true,
    anonymousSignup: true,
    noPhoneRequired: true,
    selfHostable: true,
    federated: false,
    p2p: false,
    offlineCapable: false,
    groupSize: "Up to 400 members (MLS-based)",
    voiceVideo: false,
    multiDevice: true,
    platforms: "SDK-based (React, React Native, Kotlin, Swift, JS)",
    funding: "VC-funded (a]16z, etc.)",
    jurisdiction: "USA",
    audited: true,
    userBase: "Developer ecosystem (used by Coinbase, Converse, etc.)",
    price: "Free SDK; node operators charge ~$5/100K messages",
    maxDevices: "Multiple (MLS-based multi-device)",
    rateLimiting: "Node-level enforcement. Details not fully published. MLS group operations have inherent overhead that limits throughput. Node operators can set policies. No ZK-based or protocol-level spam protection.",
    offlineDelivery: "Node operators store encrypted messages. Recipient retrieves from network on reconnect. Node operators paid (~$5/100K messages) — economic incentive for storage built into protocol from the start.",
    pros: [
      "Quantum-resistant encryption (MLS, audited by same firm as Signal)",
      "Identity-agnostic: wallets, passkeys, social accounts",
      "Native digital currency integration in conversations",
      "No API keys or kill switches — permissionless protocol",
      "Used by major Web3 companies (Coinbase Wallet, etc.)",
      "Node operators paid directly (~$5/100K messages)",
    ],
    cons: [
      "Primarily SDK/developer tool, not a consumer app",
      "Web3-native = requires crypto literacy for full use",
      "VC-funded (a16z) — potential conflict with decentralization ethos",
      "Still early-stage, ecosystem maturing",
      "No voice/video",
      "US jurisdiction",
    ],
    centralizationPoints: [
      "VC funding from a16z may influence direction",
      "XMTP Labs controls core development",
      "Node network still growing — early distribution may be concentrated",
      "US jurisdiction for core team",
    ],
  },
  {
    name: "Delta Chat",
    category: "federated",
    tagline: "Chat over email — uses existing email servers",
    icon: "💬",
    protocol: "Autocrypt (OpenPGP-based) over IMAP/SMTP",
    architecture: "Federated (email infrastructure)",
    encryption: "E2E (Autocrypt/OpenPGP, opt-in with verified groups)",
    openSource: "Client: Yes (Rust core + multiple frontends)",
    openSourceStatus: true,
    identifier: "Email address",
    metadataProtection: "Weak — email servers see all metadata, subject lines, etc.",
    pfs: false,
    postCompromise: false,
    quantumResistant: false,
    anonymousSignup: true,
    noPhoneRequired: true,
    selfHostable: true,
    federated: true,
    p2p: false,
    offlineCapable: true,
    groupSize: "No hard limit; practical max ~50 (email threading overhead)",
    voiceVideo: false,
    multiDevice: true,
    platforms: "Android, iOS, Windows, macOS, Linux",
    cellTooltips: {
      pfs: "OpenPGP (used by Autocrypt) relies on long-lived key pairs with no ephemeral derivation. Compromising your PGP key exposes all past and future messages.",
    },
    funding: "Grants (Open Technology Fund, NLnet, etc.) + donations",
    jurisdiction: "Germany/EU (non-profit)",
    audited: true,
    userBase: "~100K+",
    price: "Free (open source)",
    maxDevices: "Multiple (IMAP/SMTP-based; each device polls independently)",
    rateLimiting: "Email server rate limits apply. Gmail: ~500 msgs/day, most SMTP servers have per-hour/per-day caps. Autocrypt key exchange adds overhead. Effectively inherits decades of email anti-spam infrastructure (SPF, DKIM, DMARC, greylisting).",
    offlineDelivery: "Email infrastructure. Message sits in recipient's IMAP inbox on their email server. 40+ years of battle-tested store-and-forward. Robust and reliable, but email server sees everything.",
    pros: [
      "Works with ANY existing email account — zero new infrastructure",
      "Recipient doesn't need Delta Chat installed (gets regular email)",
      "Massive existing federation (all email servers in the world)",
      "Familiar chat UX on top of proven email infrastructure",
      "Guaranteed delivery and offline support (email stores messages)",
      "Webxdc apps — run mini-apps inside chats",
    ],
    cons: [
      "Email metadata is highly exposed (servers, providers, governments)",
      "OpenPGP lacks PFS and post-compromise security",
      "Autocrypt key exchange can be MITMed without verification",
      "Delivery can be slow (email polling intervals)",
      "Spam filters may interfere with messages",
      "No voice/video calls",
      "Email providers can read unencrypted messages and metadata",
    ],
    centralizationPoints: [
      "Relies on user's email provider (Gmail, Outlook, etc.)",
      "Major email providers (Google, Microsoft) dominate infrastructure",
      "DNS system for email routing",
      "Email provider can block or filter messages",
    ],
  },
  {
    name: "Discord",
    category: "centralized",
    tagline: "Gaming-born community platform, no E2EE, 500M+ users",
    icon: "🎮",
    protocol: "Proprietary (WebSocket for messaging, WebRTC for voice/video)",
    architecture: "Centralized (Discord Inc. servers)",
    encryption: "TLS in transit only — no E2EE; Discord can read all messages and attachments",
    openSource: "Client: No, Server: No",
    openSourceStatus: false,
    identifier: "Email required (phone optional for 2FA)",
    metadataProtection: "None — Discord stores all messages, can read all content, collects device info, IP addresses, and behavioral data",
    pfs: false,
    postCompromise: false,
    quantumResistant: false,
    anonymousSignup: false,
    noPhoneRequired: true,
    selfHostable: false,
    federated: false,
    p2p: false,
    offlineCapable: false,
    groupSize: "Servers: 500,000+ members; group DMs: up to 10",
    voiceVideo: true,
    multiDevice: true,
    platforms: "Android, iOS, Windows, macOS, Linux, Web",
    funding: "VC-backed (Index Ventures, Greenoaks, etc.) + Discord Nitro subscriptions",
    jurisdiction: "USA (Discord Inc., San Francisco) — Five Eyes",
    audited: false,
    userBase: "~500M+ registered accounts, ~200M monthly active users",
    rateLimiting: "Published: 50 requests/sec global per user. Message sending: ~5 msgs/5 sec before rate limiting. Bot endpoints have granular per-route limits. Server admins can set per-channel slowmode (0–6h between messages).",
    offlineDelivery: "Discord cloud stores all messages permanently (until manually deleted). Full history available on reconnect across all devices. No encryption barrier — Discord has access to all stored content.",
    pros: [
      "Dominant platform for gaming and online communities — massive network effect",
      "Rich feature set: servers, channels, threads, stages, voice rooms, screen share",
      "Bots and integrations ecosystem (tens of thousands of bots)",
      "Reliable voice/video with low latency (WebRTC)",
      "Free with generous limits; Nitro for extras",
      "Good UX across all platforms including web",
      "Role-based permissions and moderation tools",
    ],
    cons: [
      "Zero end-to-end encryption — Discord can read every message",
      "Fully closed source — no transparency into client or server",
      "US jurisdiction (Five Eyes) — subject to law enforcement requests",
      "Extensive data collection: message content, voice activity, device info, IP",
      "No self-hosting or federation — pure walled garden",
      "Account requires email; real identity often linkable",
      "Past data breaches (2023: 760M records leaked via Spy.pet scraping)",
      "History of banning servers and users without clear appeal process",
    ],
    centralizationPoints: [
      "All messages stored and readable by Discord Inc.",
      "Discord controls account creation, banning, and content moderation globally",
      "No self-hosting — you cannot run your own Discord",
      "US jurisdiction with CLOUD Act exposure",
      "App Store / Google Play distribution",
    ],
  },
  {
    name: "Telegram",
    category: "centralized",
    tagline: "900M+ users, cloud messaging, E2EE only in Secret Chats",
    icon: "✈️",
    protocol: "MTProto 2.0",
    architecture: "Centralized (Telegram cloud servers)",
    encryption: "Transport encryption by default; E2EE only in Secret Chats (not groups or regular chats)",
    openSource: "Client: Yes, Server: No",
    openSourceStatus: null,
    identifier: "Phone number required",
    metadataProtection: "Weak — Telegram servers store and can access all non-Secret-Chat messages; contact lists, group memberships, and message content visible to Telegram",
    pfs: null,
    postCompromise: false,
    quantumResistant: false,
    anonymousSignup: false,
    noPhoneRequired: false,
    selfHostable: false,
    federated: false,
    p2p: false,
    offlineCapable: false,
    groupSize: "Up to 200,000 members (groups); unlimited subscribers (channels)",
    voiceVideo: true,
    multiDevice: true,
    platforms: "Android, iOS, Windows, macOS, Linux, Web",
    cellTooltips: {
      pfs: "Regular chats and all group chats use transport-only encryption (MTProto) with no PFS. Secret Chats do use ephemeral keys with PFS, but they're opt-in and unavailable for groups.",
      openSourceStatus: "Client apps are open source; the server code is fully closed — there is no way to verify server-side privacy or security claims independently.",
    },
    funding: "Telegram Premium subscription + planned ad revenue in public channels",
    jurisdiction: "UAE (Dubai) — moved from Russia via UK, BVI, Singapore",
    audited: false,
    userBase: "~900M+ monthly active users",
    price: "Free / Telegram Premium ~$4.99/mo",
    maxDevices: "Unlimited (cloud-based sync across all devices)",
    rateLimiting: "Server-side, not fully published. Bot API limits: 30 msg/sec to groups, 1 msg/sec per chat for bots. Forward limit: 5 chats at once for non-premium users to curb viral spread. Account banning for spam behavior.",
    offlineDelivery: "Telegram cloud stores all messages indefinitely (except Secret Chats, which are device-local). Messages sync instantly across all devices on reconnect. Cloud storage is the feature — and the privacy risk.",
    pros: [
      "Largest user base of any privacy-adjacent messenger (~900M MAU)",
      "Cloud sync — messages available on any device, any time",
      "Massive group support (200,000 members) and channels for broadcasting",
      "Rich feature set: bots, stickers, polls, threads, mini-apps",
      "MTProto 2.0 is reasonably fast and efficient",
      "Secret Chats provide device-to-device E2EE with PFS when needed",
      "Open-source clients, multiple third-party clients available",
    ],
    cons: [
      "Regular chats and groups are NOT end-to-end encrypted — Telegram can read them",
      "No PFS or post-compromise security for regular chats",
      "Phone number required — no anonymous sign-up",
      "Closed server code — no way to verify server-side privacy claims",
      "No independent audit of MTProto implementation",
      "UAE jurisdiction — Pavel Durov's arrest in France (Aug 2024) raised content moderation concerns",
      "Metadata: who you talk to, when, group memberships all visible to Telegram",
      "Secret Chats not available on desktop in all configurations",
    ],
    centralizationPoints: [
      "All messages (except Secret Chats) stored on Telegram's servers",
      "Telegram LLC / TG Inc. controls all infrastructure",
      "Phone number as mandatory identifier",
      "Server code is closed source — no independent verification",
      "Pavel Durov holds effective control over the company",
    ],
  },
  {
    name: "WhatsApp",
    category: "centralized",
    tagline: "2B+ users, Signal Protocol E2EE, owned by Meta",
    icon: "📱",
    protocol: "Signal Protocol (modified, with multi-device via MLS-like key distribution)",
    architecture: "Centralized (Meta/WhatsApp servers)",
    encryption: "E2E (Signal Protocol) — default for all messages and calls",
    openSource: "Client: No, Server: No",
    openSourceStatus: false,
    identifier: "Phone number required",
    metadataProtection: "Weak — Meta harvests extensive metadata: who you contact, frequency, duration, device info, IP address, linked Facebook/Instagram accounts",
    pfs: true,
    postCompromise: true,
    quantumResistant: false,
    anonymousSignup: false,
    noPhoneRequired: false,
    selfHostable: false,
    federated: false,
    p2p: false,
    offlineCapable: false,
    groupSize: "Up to 1,024 members",
    voiceVideo: true,
    multiDevice: true,
    platforms: "Android, iOS, Windows, macOS, Web",
    funding: "Meta Platforms (ad-driven parent company; WhatsApp itself has no ads currently)",
    jurisdiction: "USA (Meta Platforms, Inc.) — Five Eyes",
    audited: false,
    userBase: "~2B+ monthly active users",
    price: "Free (owned by Meta)",
    maxDevices: "5 (phone + up to 4 linked companion devices)",
    rateLimiting: "Server-side. Forward limit: max 5 chats at once (reduced from 250 after misinformation concerns). Bot/business API has published rate limits. Account banning for automated or spammy behavior.",
    offlineDelivery: "WhatsApp servers store encrypted messages for offline recipients (up to 30 days). Multi-device syncs via companion device protocol. Cloud backups to Google Drive / iCloud available — historically unencrypted, now optionally E2EE.",
    pros: [
      "Largest E2EE messenger by volume — ~2B MAU",
      "Signal Protocol provides strong, well-audited message encryption by default",
      "Voice and video calls are E2EE",
      "Multi-device support (up to 4 linked devices)",
      "Near-universal adoption — best chance of your contacts already being on it",
      "Free, polished UX across all major platforms",
    ],
    cons: [
      "Owned by Meta — extensive metadata harvesting shared with Facebook ecosystem",
      "Phone number required — no anonymous sign-up",
      "Fully closed source — neither client nor server is open",
      "No independent audit of WhatsApp's Signal Protocol implementation",
      "US jurisdiction (Five Eyes) — subject to FISA court orders",
      "Cloud backups historically unencrypted (E2EE backup opt-in since 2021)",
      "Business messaging (WhatsApp Business API) messages may be readable by Meta partners",
      "No self-hosting or federation",
    ],
    centralizationPoints: [
      "All message routing through Meta/WhatsApp servers",
      "Meta Platforms controls all client and server code",
      "Phone number as mandatory identifier",
      "Metadata integrated into Meta's ad-targeting infrastructure",
      "No independent verification of encryption implementation",
    ],
  },
  // ── New entries ────────────────────────────────────────────────────────────────
  {
    name: "Threema",
    category: "centralized",
    tagline: "Swiss-made, paid app — anonymous by default, no phone required",
    icon: "🟢",
    protocol: "Threema Protocol (NaCl/libsodium, Curve25519 ECDH + XSalsa20)",
    architecture: "Centralized (Swiss servers)",
    encryption: "E2E (NaCl — Curve25519 key agreement, XSalsa20-Poly1305)",
    openSource: "Client: Yes (since 2020), Server: No",
    openSourceStatus: null,
    identifier: "Random 8-char Threema ID — phone/email optional and never shared",
    metadataProtection: "Good — minimal metadata stored; contact lists hashed and never uploaded; IP-based, not account-linked",
    pfs: true,
    postCompromise: false,
    quantumResistant: false,
    anonymousSignup: true,
    noPhoneRequired: true,
    selfHostable: false,
    federated: false,
    p2p: false,
    offlineCapable: false,
    groupSize: "Up to 1,000 members",
    voiceVideo: true,
    multiDevice: true,
    platforms: "Android, iOS, Windows, macOS, Linux, Web (Threema Web)",
    cellTooltips: {
      openSourceStatus: "Client open-sourced in 2020 with reproducible builds; server code remains proprietary — Threema GmbH's server infrastructure cannot be independently verified.",
    },
    funding: "One-time purchase (~$5.99) — no ads, no subscriptions, no data monetization",
    jurisdiction: "Switzerland (Threema GmbH) — not Five Eyes",
    audited: true,
    userBase: "~13M+ users",
    price: "~$5.99 one-time purchase (iOS/Android)",
    maxDevices: "Multiple (Threema Web + desktop app)",
    rateLimiting: "Server-side, not published. Centralized infrastructure with standard DDoS and abuse controls. No protocol-level spam protection details disclosed publicly.",
    offlineDelivery: "Swiss servers queue messages for offline recipients. Standard centralized store-and-forward. Messages deleted from server after delivery confirmation.",
    pros: [
      "No phone number or email required — fully anonymous sign-up",
      "One-time purchase model aligns incentives away from data monetization",
      "Swiss jurisdiction — strong privacy laws, not Five Eyes",
      "Peer-reviewed NaCl cryptography — well-understood and audited",
      "Open-source clients since 2020, reproducible builds",
      "Threema Work for enterprise on-premises option",
      "Minimal metadata: contact lists never uploaded to server",
    ],
    cons: [
      "Centralized Swiss servers — single point of control",
      "Server-side code is closed source",
      "No Perfect Forward Secrecy for group messages",
      "Paid app creates barrier vs. free alternatives",
      "Smaller network effect compared to Signal or WhatsApp",
      "No federation or self-hosting for personal use",
      "Relatively small user base outside German-speaking countries",
    ],
    centralizationPoints: [
      "Threema GmbH operates all servers in Switzerland",
      "Server code is proprietary and closed",
      "Push notifications via Google/Apple (circumventable via polling)",
      "App distribution via App Store / Google Play",
    ],
  },
  {
    name: "Olvid",
    category: "centralized",
    tagline: "No phone, no email, no directory — French ANSSI-certified E2EE",
    icon: "🔒",
    protocol: "Olvid Protocol (custom, formally verified — OBAKE / CHIKARA / SOLITUDE)",
    architecture: "Centralized (optional server for async delivery only)",
    encryption: "E2E (formally verified custom protocol — no central key escrow)",
    openSource: "Client: Yes (since 2021)",
    openSourceStatus: null,
    identifier: "Cryptographic identity — no central directory, no phone, no email",
    metadataProtection: "Very strong — server never knows who talks to whom; contact graph is completely hidden",
    pfs: true,
    postCompromise: true,
    quantumResistant: false,
    anonymousSignup: true,
    noPhoneRequired: true,
    selfHostable: false,
    federated: false,
    p2p: false,
    offlineCapable: false,
    groupSize: "Up to 50 members",
    voiceVideo: true,
    multiDevice: true,
    platforms: "Android, iOS, Windows, macOS",
    cellTooltips: {
      openSourceStatus: "Client open-sourced in 2021 with reproducible builds; server code is proprietary. The server only routes ciphertext and cannot read content, but the code cannot be independently audited.",
    },
    funding: "SAS (private company) — freemium consumer, paid enterprise (Olvid for Business)",
    jurisdiction: "France (EU) — GDPR-compliant",
    audited: true,
    userBase: "Small — primarily French enterprise and government",
    price: "Free (consumer) / Paid enterprise (Olvid for Business)",
    maxDevices: "Multiple (up to 5 linked devices per identity)",
    rateLimiting: "Server-side, not published. Standard infrastructure controls. Server only routes ciphertext — cannot inspect content.",
    offlineDelivery: "Encrypted messages queued on Olvid servers until recipient reconnects. Server has no visibility into plaintext or metadata beyond routing ciphertext to the correct cryptographic identity.",
    pros: [
      "CSPN certification by ANSSI (French national cybersecurity agency) — rare official certification",
      "Formally verified cryptographic protocol — mathematical proofs of security properties",
      "Zero contact discovery via server — server cannot map social graph",
      "No phone, email, or any personal info required to register",
      "Post-compromise security — ratcheting protocol heals after key compromise",
      "EU jurisdiction with strong GDPR protections",
      "Open-source clients with reproducible builds",
    ],
    cons: [
      "Centralized servers for message routing (though server is blind to content)",
      "Server code is not open source",
      "Very small user base — limited network effects",
      "Contact exchange requires out-of-band QR scan or invite link — no phone book lookup",
      "Group size limited (~50 members)",
      "Primarily a French/enterprise product — English UX is secondary",
      "No self-hosting option for personal accounts",
    ],
    centralizationPoints: [
      "Olvid SAS operates routing servers in France",
      "Server source code is proprietary",
      "App distribution via App Store / Google Play",
      "Company controls protocol evolution",
    ],
  },
  {
    name: "Wire",
    category: "centralized",
    tagline: "Signal-Protocol roots, MLS for enterprise, email or phone sign-up",
    icon: "⚪",
    protocol: "Proteus (Signal Protocol fork) for 1:1 + MLS (RFC 9420) for groups",
    architecture: "Centralized (cloud) or self-hosted (Wire for Enterprise)",
    encryption: "E2E (Proteus for DMs, MLS for group chats)",
    openSource: "Client: Yes, Server: Partially (Wire Server on GitHub)",
    openSourceStatus: null,
    identifier: "Phone number or email address",
    metadataProtection: "Moderate — server sees who communicates with whom; metadata retained per Swiss law",
    pfs: true,
    postCompromise: true,
    quantumResistant: false,
    anonymousSignup: false,
    noPhoneRequired: true,
    selfHostable: true,
    federated: false,
    p2p: false,
    offlineCapable: false,
    groupSize: "Up to 500 members (consumer); configurable for enterprise",
    voiceVideo: true,
    multiDevice: true,
    platforms: "Android, iOS, Windows, macOS, Linux, Web",
    cellTooltips: {
      openSourceStatus: "Client is open source; Wire Server is partially on GitHub but not all server components are public — some infrastructure code remains proprietary.",
    },
    funding: "VC-backed (Wire Swiss GmbH) — B2B enterprise revenue model",
    jurisdiction: "Switzerland (Wire Swiss GmbH) with US corporate structure (Wire Group Holdings)",
    audited: true,
    userBase: "~1M-5M users (consumer) + enterprise deployments",
    price: "Free (consumer) / Wire for Business from ~€4/user/mo",
    maxDevices: "8 linked devices",
    rateLimiting: "Server-side, not fully published. Standard infrastructure controls. MLS group operations have inherent computational overhead. Enterprise deployments can configure their own limits.",
    offlineDelivery: "Centralized cloud servers queue messages for offline recipients. Enterprise self-hosted deployments run their own Wire Server instances. Standard store-and-forward on sender's connected server.",
    pros: [
      "MLS (RFC 9420) adoption for group encryption — standards-based and well-reviewed",
      "Self-hostable enterprise option — full control for organizations",
      "Email sign-up available — no phone number required",
      "Open-source clients with reproducible builds",
      "Audited by Trail of Bits",
      "Swiss jurisdiction with strong privacy laws",
      "Voice/video conferencing built in",
    ],
    cons: [
      "Requires phone or email — no truly anonymous sign-up",
      "US corporate ownership (Wire Group Holdings) despite Swiss operations",
      "Centralized servers for consumer tier — no federation",
      "Business model pivot toward enterprise may deprioritize consumer app",
      "Server code is only partially open source",
      "Smaller consumer user base vs. Signal",
      "Past ownership changes raised trust questions (moved from US to Swiss to mixed)",
    ],
    centralizationPoints: [
      "Wire cloud servers handle all consumer message routing",
      "Wire Swiss GmbH / Wire Group Holdings control development",
      "App Store / Google Play distribution",
      "DNS and PKI infrastructure for identity verification",
    ],
  },
  {
    name: "BChat (Beldex)",
    category: "decentralized",
    tagline: "Session-like design on Beldex blockchain — origin unclear, treat with caution",
    icon: "⚠️",
    warning: "Project provenance is unclear. No independent security audit. Limited transparency about team and funding. Exercise caution before trusting with sensitive communications.",
    protocol: "BChat Protocol (reportedly Signal Protocol-based, on Beldex master node network)",
    architecture: "Decentralized (Beldex master node network — Monero fork)",
    encryption: "E2E (claimed Signal Protocol-based)",
    openSource: "Client: Claimed open source — independent review limited",
    openSourceStatus: null,
    identifier: "Random BChat ID (similar to Session's approach)",
    metadataProtection: "Claimed strong — similar design to Session with onion routing",
    pfs: false,
    postCompromise: false,
    quantumResistant: false,
    anonymousSignup: true,
    noPhoneRequired: true,
    selfHostable: false,
    federated: false,
    p2p: false,
    offlineCapable: false,
    groupSize: "~100 (claimed; unverified, similar to Session)",
    voiceVideo: true,
    multiDevice: true,
    platforms: "Android, iOS, Windows, macOS",
    cellTooltips: {
      openSourceStatus: "Client is claimed open source but independent review is severely limited — provenance is unclear and no credible audit exists.",
    },
    funding: "Beldex token (BDX) ecosystem — unclear beyond token economics",
    jurisdiction: "Unknown (Singapore / Seychelles — unverified)",
    audited: false,
    userBase: "Very small — unverified figures",
    price: "Free",
    maxDevices: "Multiple (claimed, unverified)",
    rateLimiting: "Unclear. Relies on Beldex master node network for routing. No published rate limiting details. No independent verification of spam protection claims.",
    offlineDelivery: "Claimed similar to Session: messages stored in master node swarms for a limited period. No independent verification of implementation.",
    pros: [
      "No phone or email required for sign-up",
      "Claims onion routing for IP privacy",
      "Decentralized node network (if functioning as claimed)",
      "No phone number identity",
    ],
    cons: [
      "No independent security audit — unverified security claims",
      "Unclear team identity and funding structure",
      "Jurisdiction unknown — no legal accountability",
      "No PFS or post-compromise security",
      "Beldex token economics may conflict with long-term sustainability",
      "Very small and unverified user base",
      "Appears to be largely derivative of Session without transparent improvements",
      "Limited community, support, and ecosystem",
    ],
    centralizationPoints: [
      "Beldex master nodes controlled by token stakers",
      "Unknown development team controls codebase",
      "Token economics dictate node incentives",
      "No verifiable governance or accountability structure",
    ],
  },
  {
    name: "Keet",
    category: "p2p",
    tagline: "True P2P on Hypercore — no servers, no metadata, voice/video built in",
    icon: "🟢",
    protocol: "Hypercore Protocol (Holepunch) + Noise Protocol (XX pattern)",
    architecture: "Pure P2P (DHT-based peer discovery, direct device-to-device)",
    encryption: "E2E (Noise Protocol + XSalsa20/libsodium)",
    openSource: "Underlying stack: Yes (Pear Runtime, Hypercore/Holepunch). Keet app client: No — no public repo",
    openSourceStatus: null,
    identifier: "Cryptographic seed phrase (24 words) — no phone, email, or account",
    metadataProtection: "Strong — no central server, no metadata collected; IP addresses potentially visible to group peers via P2P connections",
    pfs: null,
    postCompromise: null,
    quantumResistant: false,
    anonymousSignup: true,
    noPhoneRequired: true,
    selfHostable: false,
    federated: false,
    p2p: true,
    offlineCapable: false,
    groupSize: "Not published — no documented limit",
    voiceVideo: true,
    multiDevice: true,
    platforms: "Android, iOS, Windows, macOS, Linux, Web",
    cellTooltips: {
      openSourceStatus: "The underlying Pear Runtime and Hypercore/Holepunch protocol libraries are open source. The Keet app client itself is not — no public source repository exists for the application code, so the actual messenger is effectively closed source.",
      pfs: "Noise Protocol (XX pattern) provides forward secrecy for session establishment. Per-message ratcheting equivalent to Signal's Double Ratchet is not documented — PFS guarantees for ongoing messages are unconfirmed.",
      postCompromise: "Post-compromise 'healing' properties are not documented for Keet's Hypercore-based encryption. Treat as unconfirmed.",
    },
    funding: "Tether ($10M+) + Bitfinex ($10M+) — both cryptocurrency companies",
    jurisdiction: "El Salvador (Holepunch, S.A. de C.V.)",
    audited: false,
    userBase: "~380K+ downloads",
    price: "Free",
    maxDevices: "Unlimited (seed phrase syncs all linked devices)",
    rateLimiting: "None published. P2P architecture provides implicit limits via direct connections. No protocol-level spam protection — contact acceptance is the only gate.",
    offlineDelivery: "No store-and-forward server. Both peers must be online for delivery. Messages are queued locally and delivered when peer reconnects, but there is no server-side buffer.",
    pros: [
      "True serverless P2P — no infrastructure to surveil or shut down",
      "No phone, email, or account required — seed phrase identity only",
      "Voice, video, and screen sharing all P2P",
      "Multi-device sync via seed phrase",
      "No metadata collection at all — Holepunch has nothing to hand over",
      "Built on the open-source Pear Runtime and Hypercore/Holepunch protocol libraries",
      "Free forever with no ads or subscriptions",
      "Runs on all major platforms including mobile",
    ],
    cons: [
      "Keet app client is not open source — no public repo for the application code",
      "No published independent security audit",
      "PFS and post-compromise security unconfirmed — not documented",
      "IP addresses potentially visible to group peers (P2P architecture)",
      "No offline delivery — both peers must be online simultaneously",
      "Backed by Tether and Bitfinex — cryptocurrency companies with regulatory controversy",
      "El Salvador jurisdiction — unusual legal context for privacy tool",
      "DHT bootstrap nodes are a centralization point for peer discovery",
      "Group size limits undocumented — scalability unproven",
      "Small user base — limited network effects",
    ],
    centralizationPoints: [
      "DHT bootstrap and relay nodes operated by Holepunch for peer discovery",
      "Holepunch (Tether/Bitfinex-backed) controls app development",
      "App Store / Google Play distribution",
      "Tether/Bitfinex funding may influence protocol direction",
    ],
  },
  {
    name: "Towns",
    category: "web3",
    tagline: "Decentralized community spaces on blockchain — Discord alternative for DAOs",
    icon: "🏘️",
    protocol: "River Protocol (MLS-based E2EE over custom blockchain transport)",
    architecture: "Decentralized (River Protocol on Base L2 / custom chain)",
    encryption: "E2E (MLS — RFC 9420) for DMs; public channels unencrypted",
    openSource: "Protocol: Yes (River Protocol), Client: Partially",
    openSourceStatus: null,
    identifier: "Ethereum wallet address (EVM-compatible)",
    metadataProtection: "Moderate — on-chain membership is public; DM content encrypted, channel membership visible on-chain",
    pfs: true,
    postCompromise: true,
    quantumResistant: false,
    anonymousSignup: false,
    noPhoneRequired: true,
    selfHostable: false,
    federated: false,
    p2p: false,
    offlineCapable: false,
    groupSize: "Encrypted DMs: up to ~50 (MLS); public channels: thousands",
    voiceVideo: false,
    multiDevice: true,
    platforms: "Web, iOS, Android",
    cellTooltips: {
      openSourceStatus: "River Protocol is open source; the Towns client is only partially open — not all client code is publicly available.",
    },
    funding: "VC-backed (a16z crypto, Benchmark, etc.)",
    jurisdiction: "USA",
    audited: false,
    userBase: "Early stage — growing Web3/DAO community",
    price: "Free (gas costs for on-chain actions)",
    maxDevices: "Multiple (wallet-based; any device with wallet access)",
    rateLimiting: "Blockchain transaction model provides implicit rate limiting — on-chain operations have gas costs. Off-chain message relay handles throughput. No published spam protection details.",
    offlineDelivery: "River Protocol nodes store messages. On-chain state ensures membership and access control; off-chain relay nodes handle message storage and delivery. Messages retrievable on reconnect via protocol relay.",
    pros: [
      "MLS (RFC 9420) for DM encryption — modern standard, post-compromise security",
      "True ownership — community membership NFTs, DAOs can govern their spaces",
      "Discord-like UX with Web3 native features (token-gated channels, on-chain governance)",
      "Open River Protocol — others can build on the same infrastructure",
      "No phone number required — wallet-native identity",
    ],
    cons: [
      "Requires Ethereum wallet — high barrier for non-crypto users",
      "VC-backed (a16z) — large investor influence on a 'decentralization' product",
      "US jurisdiction",
      "Public channel content and membership visible on-chain",
      "No voice/video (yet)",
      "No independent security audit",
      "Token-gating and NFT mechanics add complexity",
      "Early stage — reliability and scale unproven",
    ],
    centralizationPoints: [
      "VC funding (a16z) may shape protocol direction",
      "Towns Inc. controls development and app distribution",
      "River Protocol relay nodes — currently operated by Towns",
      "Base L2 / Ethereum blockchain dependency",
      "US jurisdiction for core company",
    ],
  },
];

// ── Real brand logos from /public/logos/ ──────────────────────────────────────

export const logoFiles = {
  "Signal":                    "signal.svg",
  "Discord":                   "discord.svg",
  "Telegram":                  "telegram.svg",
  "WhatsApp":                  "whatsapp.svg",
  "Matrix / Element":          "matrix.svg",
  "Session":                   "session.svg",
  "SimpleX Chat":              "simplex.svg",
  "Briar":                     "briar.svg",
  "Status":                    "status.webp",
  "Logos Chat (Chat SDK)":     "logos.svg",
  "XMPP (Conversations, etc.)":"xmpp.svg",
  "Tox":                       "tox.svg",
  "DarkFi (DarkIRC)":          "darkfi.svg",
  "XMTP":                      "xmtp.svg",
  "Delta Chat":                "deltachat.svg",
  "Threema":                "threema.svg",
  "Olvid":                  "olvid.png",
  "Wire":                   "wire.svg",
  "BChat (Beldex)":         "bchat.svg",
  "Keet":                   "keet.svg",
  "Towns":                  "towns.svg",
};
